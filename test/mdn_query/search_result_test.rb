...
class MdnQuerySearchResultTest < Minitest::Test
  def setup
    ...
    @url = 'url'
    @search_result = ::MdnQuery::SearchResult.new(@json)
  end

  def test_empty?
    empty_document = ::MdnQuery::SearchResult.new(pages: 0)
    refute @search_result.empty?
    assert empty_document.empty?
  end

  def test_next?
    last_page = ::MdnQuery::SearchResult.new(page: 5, pages: 5)
    assert @search_result.next?
    refute last_page.next?
  end

  def test_previous?
    first_page = ::MdnQuery::SearchResult.new(pages: 5, page: 1)
